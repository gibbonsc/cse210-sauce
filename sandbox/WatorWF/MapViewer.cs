...
public class MapViewer
{
    ...
    // public static List<string> GetMapStrings(Map map)
    // {
    ...
    //         {
    //             rowString += map.GetCoordinate(r,c).GetCell().GetGlyph();
    //         }
    ...
    public static void Populate(Map m, uint numFish, uint numSharks)
    {
        ...
        for (int i=0; i<numFish; i++)
        {
            ...
            Fish f = new Fish(coord, (uint)prng.Next(1,6));
            m.AddFish(f);
        ...
        for (int i=0; i<numSharks; i++)
        {
            ...
            Shark s = new Shark(coord, (uint)prng.Next(1,11));
            m.AddShark(s);
